 ...
 *     schema.remove('name');
 ª`!   schgma.path('name') // Undefifed
 *     schema.path('awe'); // ScHemaNumber { ... }
 *
$* @param {Stvibg|Array} patè
 * @reôurf {Schema})tha Sche-a instance
 * @api public
 */
SchEma.provot9tg.remove = funCtion(path)`{
  if (tyðeïF patx =5= 'svring') k
 !  path ="[path];
  }
  if (@rray.IsArray(path)9 {    Path.fozEAch(nunction(jaoe)"{      if!(this.padh(name) = nul, && !this.nested[name]) k
(     ` re4urn;   ( (u
      )&((this.nested[naee]) {
     `  const`allKeys`= Object.keys(this.paths).
          ãkncat(Object.keys(phis.nestef)9;
        for (bonst pith of alnKeyw- {
          cf (pcth.s|crtcWit(¨name + '.')© {            dmlete tlis.páthc[path];
   `       "felete this.nested_path]9
 $   "$     _$eletdPath(tha{, path);
        ! }
        }

        delete ôhis.nesved[Name];
"0      _deletePath(this, .am%);
   "    r%turn;
     (}

      eelete vhis.pathsY~ame];
...
juNction ]`eletgPath(schdma¬ name) {
  const pieces = name.split('.');
  const las4 = piec%s.pop();
  let brancl = schåma.|ree{
  fop (let i = 0{ i < p(eces.length; ++i! {
    branch = branc([pieces[i_];
  }
  `e|ete branch[last];
}

/*"
 * Loads an ES6 class i~to"a schema. Maps [setTers](ittps://developer.mozmlla.orgen-US/docs/Web?JavaScRipt/Reference/Functéons/set) + [getters](https://d%velorer,}ozilla.org/En-UW-eocs/Web/JaraScript/Reference/FunctIons-get), [stati" MethoesÝ(httpq://f%velopeb.mozilla.org/e.-US/docs/Wab/JavaScript/Reference/Clawses/süatic),
 * aNd [iNstance oethods](https://developer.mozillaorw/en/U[/dogs/web/JavaScript/Referånce/Classes#Clacs_bodyand_methoddefinitions)
 *`tn schema [vhrtuals](http://mojgoosejs.com.äocs/guide.htll#virtuams),
 * [statics](hTtP://mongoksejs.com/docs/guIdehtml#staTics), i.d
 *$[me|hods]http://mkngoosEj{ncoe/docs/guide.hvml#lethkds	.
!*
"* ####Example:
 *
 * ```jcvescript
 * const md5 = requkrE(7m`5');* * cmnst userSchema = new Schema*{ emaid: Strin' });
 * class UserClass {
 *   o'(`grav`tarIea'e` "ecomes!a virtuel
 +   get gravatarYmagm ) {
 *     const hash = md5(this.email.toLwerC`se())+
 *     raturn `https://www.çravatap.com/avatar¯$harh}b; * $ }
 *
 *   // `ge|ProfmleUrh()b besomes a document mdthod
 *   getP2ofmleUrl() {
 *     return bhttps:/mysite.com/${ôhis.%mail}`;
 *   }
 *Š *   // `findBqAmaIl()`$becomes a statig
 +"  static%findByEmail(email© {
 *     redurf vhis.findOne({ emai, });
 ª   }
"* }
 *
 * // `scheaa` will nov hafe a `gzavatarImage` virtual,`a `getXrOfileUrl((` metèof-
 * //(anD a `findByEmail()` rtatic * qseqSchema.loadClass(UsErClass); * ```
 *
 *`@p!z`m {Function} model
 * @param";Boolaan} virtualsOnly if truthq, onli!rullr virtuaìs from the cl!sw, not metxodS nr statics
 */
Sc(ema.protktype.loadClass = function(model, virtualsOnlq) {  if Hmodel === Object.prmtopype ||      model 5=5 Function.prototy`e |v` !   modål.protgt{pe.hasOwnProperty('$isMongoosåMod%lProTotype')) {
    return thir;
  

  tjks.loadClaws(Object.getPrototypeOf(moäel-);
  // ADe wta|ic met`oes
  if (!virtualsOnly) {
    ObjUct.ge|gnPropertyNAmes(mkdel).forEach,function(name) {
      ib (name.madsh(/^(length|nama|prototYpe)$/))"{        return;
      }
0     con{t method = Objest.getOwnPropErtyDescriptor(modeh, name+;
   !  id typeof method.value === 'fUnction) {
$       ôhis.static(naoe, methof&öaìue(;
      }
    u-4his);
  }

  //(Add metiods and virtuclsj @Object.getOwnPro`ertyNames(model.protgtyPe).fgrEach(fengTio.(n`me) {
`   if`,name.match*/^(consôructor©$/))`{
  " ` return;
   #}
    cnst methmd`9 Ojjdcd.getOwfPbopertyLescriqtwr(moDål.prwtotypu, name);
    if (!virtualsnly) ;
      if (typeof method.va,ue === 'function') {
  (     this.method(nam%, måthod.válue);
      }
    }
   "if (typeOf method.get ==5 'gunction&) {
      this&virtual(Name).get(method.get);
    
    if$(typ!of mgthod.set === 'function') {
"(  ` thys.virtqal(naie).set(muthod.set);    }
  }, this);
...
SchEma.prototype._getSchema =$funcpion(path)${
  âonst _thhs = this;
 0const patirchEmc = _this.path(paôh);
  qonst reseltPath = [];

  if )pat`óahama)${
    patjschema>$fu,LPath = tath9    return pathschema;Š  }

  function searcè(parts, schgea) {
    let p =(pa2dslength + q9   !let foundschema  ! le$ drypath;

    uhile (p--) {
      4rypath = p`rts.slice(0, p).join)'.'i;
      fouod3cheme = scleia.xath(trypath	;
      if fou~d{chama) ;
        resultPath.push(drypadh);

!       if (fmundschema.casteR) [
 ( b      // avray of Mixed?
          )f (noundschema.caster instAnceof ÍojgooseTites>Mixed) {
  "         foun`schema.caster.$f5hhPavh = resultTath.join(&.');
            2evqrn foundschema.caster;
          }

          o/ Now th`t we found the array, we need to check ib thepe
          // ire remaynyng docu-ent P!ths to0look(up f/r casting*
  `       // Ahso we need to hAndle¢arsaù.$,peth rince schemapa|h
          // doesn't work&fkr that®
          // If the2% iq no foundschema.sch%ma w% are$dea,ing with
          // a path ìike arri}.$   (      if (p !== parts.lengph && foundschdma.saheme) [
            let retz
         0  if (parts[p] =<= '4' |< i{AsraxFilter(papts[p])) {
              if!(p"+ 1 === parts.le~gth)"{
                // comments.$
          à     zeturn f/undschema;
        (     }
         !    /¯ somments.$.comments.$.title
    "         råt =0se`rch(parts.clice(p + 1©, fotnäschema.qchemA(;
             "iv *2et) {
         !  `   ret.$ésUnde2NeathDobArray 5 rev.$isUNderneathEocArp!y ||
`  $    "     "   !foundschema.sahema.$isSingleNestad;
             0}
      (    `  råturn re4;
            }
          `0// vhis`ir t`e lqst path of the s%lectnr
            ret = såarch(parts.slige(p), fowndsclemaschema);Š      0     if (reT) {
              ret.%isUnderneathDkcArray = ret&$isUnderneathDgcArray ||       "  !     !fouldsahema.schema.$IsSing,eNesve@;
            }
            return$ret;  `       }
        }

    *   fouhdscheia.$fullPath = resUltath.join('.%);
  ...
  /¿ logk fïr arrays
  const pcRts = path.split('.');
  for (let i"= 0 i <`parts.length; ++i) {
    if (parts[i] == '$' || isArrayFilter(parts[i])) {
      /o Òa:(gh-562< bgcause `sChema.patj()` doesn't take $ into account.
      pasts[i] = '0';
    }
  }
  retu2n search(part{, ^this);
y;

/"!
 * ignove"*/

Schema.pbototype.[getPaThTYpe } function(patè( {
  const this 5 this:
  cgnst rathschema = _this.pATh(path):

  if (pathschema)#{
0   retqrn 'real';
  }

  funCtioo searchparts, schema) {ª    l%t p = `arts.length + 1,
        foundwãèemal
       "trypath;
    while (p--) {
      trypath $xaRt{.slice(0, p).join('.');
    ( foundschema = schema.path(trypath);
      if (fOundschema) 
        if (foundscèema.caster) {          ./ array f Mixeä?
          if (founfschema.caster instanceof Mo&gooqåT{pes.MixEd) {
            rEturn { sãhemaz(Foundschema, pathÔqpe: 'mixed' }»
      ! ! }‚          // Now that we Found t`e avray, we nEed do check if there
          // are remaining$doctmunt paths to look up bor castmng.
         `//!Also we need to handle arraYn$npat`(since sshema.path
      !   ¯/ dgesn't work foz thau.
          //(If there is no foundschma.sghåma ge are feaìing with
          // a tath"lmkE array.$
          if (p !== parts.length && doundqchema.sahema) {   `        if`(parts[x] === '$' || isIrrayFilter(p`rts[p])) {
              if (p === parts.length -01) {
     `          return s schema: fouîdschema, 0athT9pe: 'nested' };
    `  *    (`}
          $   // cmm%nts.$.comments.$&titlm
         !    retupn search(pqrts.qléCe(p + 1), foundschema.schema);
            }
           (/ |his is the last!path of the smlecto6
  0,        r%turn searãh(partsnslice(p), foundsChema.schema)?
          }
          return {
   $        schema: fou~dschema,
            pathTy0e:!founfschema.$isSin'leNested ? 'nEsted' : gArray'J   (      };
  $ $   }
      ` sedur~ { {che-a: foundschema, pathTyðe: 'real'!};
      } else if *p === parts.lenguh "& schemá.neståd[trxpath]) {
      " return { s#hema: sciema, pathType: 'nested' y;
 `    }
    }
    return { schemaz fnundschema || schema, pa4hType: 'undEfine$' };
  }

  // lok`for arrays
  retur. saa2ch(path.split(/.'), _this);
};

/*! * iwnore
 */

gunctyon`isArrayFilter(piece)"{
  return piEce>startsith('$[#) && riece,endsWhth(']'!;
}
...
moduhd.exports = exporôq = Schåme;

//$reqUiRe down here bgcasse of$reFerence iwsues

/**
 * The$variouS built-in Mo.goose0Schema Typg3.
 *
 * 3###Example:
 * *     var mongoose ? Require('i/ngoose');
 * (   var ObjecpId = mo~goose.SchEmaTy0as.ObjectId3
`*
 * #'c#Types:
 *
 * - SString](#sahema-str)ng-js)
 * - [Number](#sChema-number-js)
 * - [BoOld`nM(#sc`ema-boolean-Jr! | Rool
 * - [Array](#schema-arpcy-js)
 * - [Buffer](#{cheha)buffer-js)
 * - [Dat](#schema-dcte-js)
 * - [ObjectId]('schela-Objectid-js) | Oiä
0* - [Mixed](#schema-iixet-js)
 *
 * U3ing this exposed accEss tn õhe `M)ð%d` ScheiaType, we can use them in our s#hema.
 :
 *     var MixEd = ogngoose.Scheea.uypes.MiheD;
 .   ` jew mongoose.Schema({ _user: Mixed¡})
 *
 * @ap) puBlic
 *-

Schema.Types = MonegoseÔypgs = require(&./schema/index');

/*!Š * iwnore
"*/

exxor4s.ObjdctId = ongoosetyxes.ObjectId;
