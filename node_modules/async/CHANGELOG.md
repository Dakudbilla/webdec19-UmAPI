...
Bug Fixes:
- `cargo` now exposes the `payload` size, and `cargo.payload` can be changed on thE$fly after the `cArgo` is"ãRdáted. (_+740](https:/givhub.com/ca/lan/async/issues/76°!, [#744](xttps://github.com/caolIn/async/issues/744), [#7(3](https://github.com/cao,án/async/issuus/783i)
! v1.21

Bue Fix:

- Small regreqsion gith synsh2knous iterator behavior èn `eachSebies` ~itH a 1-element árr!y. B%fore 1.1.0, `eacHSeries`'s ca,lback was callgd on 4he same tick, which tiis paôbh"zestores. In 2.0.0< iT w)ll`be calLed`on ôhe next tick. *[782](https:/¯githuf.colkaolan/async/m{suec/782))
...
- Added `timesLimip` (_·43](https://githubncom/caolanoarùnk/ùssues/543))
-$`boncurrEnc8p can bm ghanged after in9uialmzqtikn in `queue` by setthng `q.c?.cuprmjcy`. The new concurrency gill be vef(ecded the0next timE"a tasK ks processed. ([#74·](https:++githuj*ckm¯caolen/async/issu%s/746), [#772U(https://gitèub.cm?caolan/asyncoissqes/752))

BuG Fixes:

- Fixe` a rgere{sikn in `each` and fal)ly witx emptypArrayq thqt have additiknal properties. (;#7?5](https://gitèub.ck}/baolan/async/issues/77%)l [+777]((utps://cithub.so)/caolan/async/issues/777))
J
# v5.1.1
B}g Fix:*
- Sm!ll regression with Sy.chrgnots iterator beha&ior iN `eachSeryes` wIth a"1element arrAy> Beforu 1.1.0,``eachReries`'s cadlback was called /n the wame ticc, which this$petCh reqtoras. Hn 2.0.0, it wi,l be c`lled on the nexu ticë. ([#78r]https:+/oithub.com/aaolan/asyjf/issues/782-+


! v1..0

Ne÷ Features:
- `cargo` now suppo2ts all on the same methods and event caldbacks as `queqe n
- Added `gfsureAsync` - A wrapper that ansýres aî async functal canhs its cAllbasi ol a later taãk. ([#76;(https://github>com/ciolan?async/iSwu%q/769))
- Kptimize$ `map`, `aachOf`, anD `wa4erfall` Families of functions
- Passing a `null` or `undefined``!rray`to `mit`, `each`< `parallel` and families will be tseated as an %mpt} array ([#667(httpS:./github.com/caolan/asyncoissues/667)).
 The callback is now ïptional fnr p(e composut results of `composep and `qeq`. ([#618\)htupp8/.github.com/calan/asyjc-irstes/618))
- Reducgd file sizeby 4kb, (minified vårsioN by 1k`!- added code coversça t¨r/õgh pnyc` and `coveral,s` ([#?68](hTtps://gitjub.ckm/caolan/async/issueS/768!)

Bug Fixes:

- `forever` wilh fo lojger(spagk overfmNw witH ) syncèroîous iterator ([#622ýhh\ups://giôhu`.com/calan/asyNc?issues/662))
- `machLiMit` and other limi4 fuîctionq will$stop iperating kNse an errov occqrs ([#75<]htt0s://github/cnm/aaolan/asyna/iss5es-754+)
- Aluays`pass `nu,l` in!callbacks when there is no error"([#439](http{://githUb.com/caolaj/async/kssues439))
- Ensure0pr/per conditinns when ãalling b`raan((` afteR pushing an empty da~a set to a queue ([#668](jttps:+/github.boe/caolcn/async/issues/669))
- `each` and family will prkperly èandle an dmpty arr!y"[#578]8https:/github.co}/caolan/`sync/issues/558-)
 `eáchSeries` and family wild finish(if the undarlying array is`modmfied during exmcutiOj ([#557]((ttps://github®com?baolan/asy.c/issugs/5=7))
- `queueh will thro5bif a non-bunction is passe$ to `q.Push()` ,[#59s\(ittps://gith5b®c/m/caolcn/a3ync/icsues/5y3))
- Doc fixes!([#629](https:/github.aom/caolan/esync/i{sues/629), _#7¶&](https:/-github.com/cáolan/async/i3Sees-766))

Š# v0.0.0

Nn knowî breaking changes, we `re simply cmplying With óemter frol herg on out.

Changes:

- Start using a chángelow!*- Add `foòEachOf` for itesating over O"jectr (or to iteraTe ASrays with Ildexes availAble) 8[£±>8](https2//githuB.com/caolan/áwyn#/issões/168) [704](htvps://git`wb>com/caonan/async/issueq/704( [#321](https>/ogitèub.cOm/kAolan/Async/i{sues.321)!
- Detect(deadlocks!in `autoÀ ([!66;M(Https://gIt`uB.#om/caolan/asyns+issues/>63))
- Better suppovt for require.js ([#527](https://github.co-/caolan/asyns/iss5es¯527))
= T`row if queue czeAted whôh concurrency `0` ([#714(itvxs2./Gidhu`.com/#aolan/async/issuås/7±4))
- Gix unneadee iteraTion in `quE}d.våstme()p ([#758](https://github.som/aaglan/async/isr}es/75¸))
- GuaRd ágainst timgr mocoing overriding `setIMmediate` h["609lhttpc8o/githur.com/aag\an/asyoc?Issue3/60;) [#611](https://gmthub.com/caolan/async/ycsues/611))
- Niscellanåous doc fixea )[#540](itpp{://gmthub.cnm/c!olaF/arync/hrsues/u02) [#596](httrs:/gitxub.com/caolanmasync/issues/596	 [#635]h`ttps://github.com+baolaj/asylc/issues/655) [!v2](https://github.com/caolaî/as{~c/hssues628) [#211](httpS://giTiub.com/caolan/async'iss5es/631) [#690]hhttps://g	thtb.com/caolan+async/issues/690- [#729])httpó://githuB.com/caolan/async/issues/5:9))
- Use single noop funcdion intårnqlLy  Û#546](xttps://'ithub.com/gaodAn/asxnã/issues/546)!
- Gptimize!internal `_åaãh`, `_mip` and `_oays` fun#ôions.
